...
from models import SLR_CNN, SLR_CNN_scalar
...
RUN_SAVE_DIR = os.path.join(config.HPARAMS_SEARCH_SAVE_DIR, "CNN", f"run_{run_id}")
...
def objective(trial):
    ...
    SEARCH_HPARAMS = {
        ...
        "HP_EPOCHS" : trial.suggest_categorical("epochs", [35]),
    ...
    try:
        for epoch in range(SEARCH_HPARAMS["HP_EPOCHS"]):
            ...
            if current_val_loss < best_val_loss:
                ...
                model_path = os.path.join(RUN_SAVE_DIR, f"CNN_model_trial_{trial.number}.pt")
...
def main():
    ...
    df.to_csv(os.path.join(RUN_SAVE_DIR, "CNN_optuna_results_%s_channels_with_scalars.csv" % channels_str), index = False)

    # Retrieve user attributes
    best_trial = study.best_trial
    best_model_state_dict = best_trial.user_attrs["best_model_state_dict"]
    best_model_path = os.path.join(RUN_SAVE_DIR, f"CNN_best_model_trial_{best_trial.number}.pt")
    ...
    BEST_TRIAL_MODEL_PATH = os.path.join(RUN_SAVE_DIR, "CNN_overall_best_model_%s_channels_with_scalars.pt" % channels_str)
    ...
    if train_losses is not None and val_losses is not None:
        ...
        loss_df.to_csv(
            os.path.join(
                RUN_SAVE_DIR, 
                "CNN_best_trial_losses_%s_channels_with_scalars.csv" % channels_str
            ), 
            index = False
        )
